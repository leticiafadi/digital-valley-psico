...
class Semana extends Model{
    ...
    public function horarios(){
        $horarios = DB::select('SELECT horario_semana.dia, horario_semana.horario, horario_semana.id FROM horario_semana WHERE id_semana = ? and not horario_semana.id IN (SELECT atendimento.id_horario from atendimento) ORDER BY dia, horario', [$this->id]);
        $retorno = [];
